...
struct MRSky
{
    ...
    float  bodyFill;        // 1 = paint the sun/moon body, 0 = report its mask only
};
...
float3 MRSkyColor(float3 d, MRSky s, out float above, out float discMask, out float halo)
{
    ...
    if (s.discRadius > 0.0)
    {
        ...
        if (discMask > 0.0 && s.bodyFill > 0.0)
        {
    ...
    {
        col += s.sunColor * pow(sd, s.sunFalloff) * s.sunIntensity * s.bodyFill;
    }
