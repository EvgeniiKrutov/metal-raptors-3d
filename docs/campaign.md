...
- **No turning back**: a hard invisible wall rides the camera's left view edge. Like the hard
  ...
  intro, since the plane flies in from behind it (docs/level-intro.md). **The wall is the
  player's alone.** Enemies used to ride it too, and pinning them against it is what left them
  stuck in the bottom-left corner for the rest of a level; they now fall off the frame freely and
  claw their way back (docs/enemies.md, "Keeping station in the scroller"). Only their soft
  `Contain` push remains.
